...
test("Test Loading With Darkmode ", () => {
  ...
  expect(mockDocument.body.style.backgroundImage).toEqual("url('https://example.com/pic.jpg')");
...
test("Test Loading Without Darkmode", () => {
  ...
  c.init();
  expect(mockStorage.get).toHaveBeenCalledWith(["darkMode", "backgroundImg"], expect.any(Function));
  mockStorage.get.mock.calls[0][1]({darkMode: false, backgroundImg: "https://example2.com/pic.jpg"});
  expect(mockDocument.body.style.backgroundImage).toEqual("url('https://example2.com/pic.jpg')");
  expect(mockDocument.body.style.backgroundRepeat).toEqual("no-repeat");
  expect(mockDocument.body.style.backgroundSize).toEqual("contain");
  expect(mockDocument.querySelector).toHaveBeenCalledWith("html");
